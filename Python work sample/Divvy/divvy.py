'''
This python file contains codes for data analysis
on 2014's Divvy Data Challenge based on Object Oriented Programming
to compute the total distance of each bike, the number of times 
a bike has been moved, and calculate some summary statistics
of the Divvy 2013 Dataset.

Classes:
- Location (Represents a geographic location)
- Divvy Station (Represents a single Divvy station)
- Divvy Trip (Represents a single Divvy trip)
- Divvy Data (Encapsulates the entire Divvy dataset)
...
class DivvyStation(object):
    ...
    def __repr__(self):
        ...
        """
        s = "<DivvyStation {}: {}>"
        s = s.format(self.stationID, self.name)
        return s
...
class DivvyData(object):
    ...
    def get_bike_times(self):
        ...
        for trip in self.trips:
            bikes[trip.bikeid] = bikes.get(trip.bikeid, 0) + trip.tripduration
    ...
    def get_bike_movements(self):
        ...
        prev_station = {b:None for b in self.bikeids}
        bike_movements = {b:[] for b in self.bikeids}

        for trip in self.trips:
            prev = prev_station[trip.bikeid]

            if prev is not None and trip.from_station != prev:
                # The bike has been moved between trips
                cap_diff = trip.from_station.dpcapacity - prev.dpcapacity
                mov = (prev, trip.from_station, cap_diff)
                bike_movements[trip.bikeid].append(mov)

            prev_station[trip.bikeid] = trip.to_station
...
def go(station_filename, trip_filename):
    ...
    print("The most used bike is {}, used a total of {}".format(bikeid, time_str(max_total)))
    ...
    print("The average number of times a bike was moved was {:.2f}".format(avg_moves))

    cap_diffs = []
    for b, movs in bm.items():
        for station1, station2, cap_diff in movs:
            cap_diffs.append(cap_diff)
    avg_cap_diff = sum(cap_diffs) / len(cap_diffs)
    stdev_cap_diff = math.sqrt(sum([(c-avg_cap_diff)**2 for c in cap_diffs])/len(cap_diffs))
